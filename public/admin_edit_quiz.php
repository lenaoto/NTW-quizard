...
<div class="container py-5">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="mb-0">Admin: Uredi kviz</h1>
    <a href="admin_dashboard.php" class="btn btn-outline-secondary">Natrag</a>
  </div>

  <form method="POST" action="admin_update_quiz.php">
    <input type="hidden" name="quiz_id" value="<?= $quizId ?>">

    <div class="mb-3">
      <label class="form-label">Naslov kviza</label>
    ...
    <div id="questions-section">
      <?php foreach ($questions as $qIndex => $question): ?>
        <div class="card p-3 mb-3">
          <input type="hidden" name="questions[<?= $qIndex ?>][id]" value="<?= (int)$question['id'] ?>">
          ...
          <div class="mb-2">
            ...
            <select name="questions[<?= $qIndex ?>][question_type]" class="form-select" required>
              ...
              <?php foreach ($types as $type): ?>
                <option value="<?= $type ?>" <?= $question['question_type'] === $type ? 'selected' : '' ?>>
                  <?= $type ?>
                </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="answers-section">
            ...
            <?php foreach ($question['answers'] as $aIndex => $answer): ?>
              <div class="input-group mb-2">
                <input type="hidden" name="questions[<?= $qIndex ?>][answers][<?= $aIndex ?>][id]" value="<?= (int)$answer['id'] ?>">
                <input type="text" name="questions[<?= $qIndex ?>][answers][<?= $aIndex ?>][answer_text]" class="form-control"
